...
public class DriverClass implements Serializable 
{
	...
	{
		//DriverClass net = new DriverClass(new int[] {4, 3, 3, 2});
		//TrainSet set = new TrainSet(4, 2);
		//set.addData(new double[] {2, 4, 6, 8}, new double[] {1, 1});
		//set.addData(new double[] {1, 3, 5, 7}, new double[] {0, 0});
		//set.addData(new double[] {2, 4, 5, 7}, new double[] {1, 0});
		//set.addData(new double[] {2, 1, 4, 7}, new double[] {0.5, 0.5});
		//net.train(set, 10000, 4);
		
		try {
			DriverClass net = DriverClass.loadNetwork("test.txt");
	...
	public static DriverClass loadNetwork(String file) throws Exception
	{
		File f = new File(file);
		ObjectInputStream out = new ObjectInputStream(new FileInputStream(f)); 
		DriverClass net = (DriverClass) out.readObject();
		out.close();
		return net;
	}
